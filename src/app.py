...
API_KEY = os.environ.get("UNIFI_API_KEY", "")  # unused in installer; kept for compatibility
...
def _login_with_password():
    ...
    if not (UNAME and UPASS and BASE):
        raise RuntimeError("Username/password login requested but UNIFI_ADDR/UNIFI_USERNAME/UNIFI_PASSWORD not set")
...
def list_cameras():
    ...
    r = _get(f"{BASE}/proxy/protect/api/cameras")
    raw = r.json()
    return _normalize_cam_list(raw)
...
def cameras_endpoint():
    ...
    cams = [{
        ...
        "name": nice_name(c),
        "model": (c.get("marketName") or c.get("type") or c.get("modelKey"))
